...
3. Set `route` to `#/projects/<slug>` and `code` to `SYS / <PROJECT>`.
...
8. Run `npm run validate` and `npm run build` before creating a checkpoint.
...
- IDs, slugs and routes must be unique.
